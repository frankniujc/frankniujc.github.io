...
<div class="row justify-content-center text-center">
  ...
  <span style="margin-right:5px; display:inline-block;">
  <p>
    <a href="https://github.com/frankniujc/entrainment" class="inline-block"><img height="153px" width="153px" src="/research/acl2025/code-thumb.png" style="border:1px solid" alt="Code thumbnail" data-nothumb=""></img>GitHub</a>
  </p>
  </span>
  <span style="margin-right:5px; display:inline-block;">
  <p>
...
**Experimental Setup**: We provide the model with two types of input: (1) without context, and (2) with context; and three context types: **related**, **irrelevant**, or **random** (counterfactual (ðŸ˜ˆ) cases will be discussed in the next part). We observe the output (logit or probability) of the **correct** (ðŸ˜Š) and the **distracting** (ðŸ˜µâ€ðŸ’«) token.[^1]
...
<div class="row justify-content-center text-center">
  <div style="display: flex; justify-content: center; align-items: center;">
    <span style="margin-right:5px; display:inline-block;">
      ...
      <p class="text-center" style="font-size: 10pt;">Average logits for different tokens across prompt settings. The colour indicates the token type (<b style="color:#1F77B4">blue</b>:ðŸ˜Š, <b style="color:#E67E22">orange</b>:ðŸ˜µâ€ðŸ’«), and the shade indicates the context setting (light: with context, dark: w/o context). **Observation**: The logits of the ðŸ˜Š tokens increase in the relevantâ†‘ setting and decrease in the irrelevantâ†“ and randomâ†“ settings, while the logits of the ðŸ˜µâ€ðŸ’« tokens increase across all settingsâ†‘.</p>
    </span>
    <span style="margin-right:5px; display:inline-block;">
...
<div class="row justify-content-center text-center">
  ...
  <span style="margin-right:5px; display:inline-block;">
    <p>
      <span style="display:inline-block; width:15px; height:15px; background-color:#1F77B4; margin-right:5px;"></span>: <Latex formula="\Delta_{p}(ðŸ˜Š) = \frac{p(\text{ðŸ˜Š w/ ctx}) - p(\text{ðŸ˜Š w/o ctx})}{p(\text{ðŸ˜Š w/o ctx})}" />
    </p>
    <p>
      <span style="display:inline-block; width:15px; height:15px; background-color:#E67E22; margin-right:5px;"></span>: <Latex formula="\Delta_{p}(ðŸ˜µâ€ðŸ’«) = \frac{p(\text{ðŸ˜µâ€ðŸ’« w/ ctx}) - p(\text{ðŸ˜µâ€ðŸ’« w/o ctx})}{p(\text{ðŸ˜µâ€ðŸ’« w/o ctx})}" />
    </p>
...
<div class="row justify-content-center text-center">
  ...
  <span style="margin-right:5px; display:inline-block; width: 1500px;">
    <p>While the probabilities and logits of the distracting tokens (ðŸ˜µâ€ðŸ’«) consistently increase, the direction of change for the correct token (ðŸ˜Š) varies based on the context type.</p>
    <p>Again, The logits of the ðŸ˜Š tokens increase in the relevantâ†‘ setting and decrease in the irrelevantâ†“ and randomâ†“ settings.</p>
  </span>
  </div>
</div>
...
We present the model with two types of distracting context prompts: a factual one (`Japan is in Asia`) and a counterfactual one (`Japan is in Africa`). Following the context prompt, we query the LM with a question (`Greece is located in ___`) and observe how it changes the logits and probabilities of the counterfactual ðŸ˜ˆ token (`Africa`), the distracting ðŸ˜µâ€ðŸ’« token (`Asia`), and the correct ðŸ˜Š token (`Europe`).
...
However, after "turning off" the entrainment heads, the contextual entrainment effect is substantially attenuated. The rankings of the tokens Berlin and Moscow dropped from 2nd and 4th to 53rd and 68th, respectively. Additionally, the difference in logits between the correct ðŸ˜Š token (`Abu`)[^2] and the distracting ðŸ˜µâ€ðŸ’« tokens (`Berlin` and `Moscow`) increased from 0.86 and 2.00 to 7.54 and 7.79, respectively. 
...
capital city relation has a small to negligible effect on other LM capabilities. This table compares the strict (answer in top-3) and credulous (answer in top-10) accuracy of the original model (ðŸ¤–) and the model with country--capital city entrainment heads removed (ðŸ”§). Removing these heads has a negligible effect on the LM's performance across other relations, with no obvious differences between ðŸ¤– and ðŸ”§.</p>
</div>
