...
Now, you should get the same results as [quiz 6](/teaching/csc485-f24/run_lm).
...
When the model reaches a hook's designated location as specified by `act_name`, the corresponding hook function in `fwd_hooks` is called with the current activation value. This lets you modify the activation in place before the model proceeds with the next layer.
